...

{
    ...
    private val prefs = defaultPrefs()
    ...
    fun provideHS(): Observable<HSResult?>? {
        ...
        var hsResult: Observable<HSResult?>? = null
        if (TextUtils.isEmpty(playerId)) {
            OneSignal.idsAvailable { userId, _ ->
                prefs[PLAYER_ID] = userId
                hsResult = reqHs(playerId)
            }
        } else {
            hsResult = reqHs(playerId)
        }
    ...
    private fun reqHs(userId: String): Observable<HSResult?>? {
    ...
    private fun reqFeeds(): Observable<FeedResponse?>? {
        return dataManager.requestFeeds(FlavorConstants.QUERY_HS_APP_ID_DYNAMICS, prefs[PLAYER_ID]!!, prefs["createFeed"])
            .subscribeOn(schedulerProvider.io())
            .observeOn(schedulerProvider.ui())
            .map { result -> result }
    }
